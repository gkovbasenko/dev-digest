...
**Fix:** add a webpack `resolve.extensionAlias` in `next.config.mjs` (`{ ".js": [".ts", ".tsx", ".js"] }`) — the general fix the 07-08 entry flagged as an alternative and skipped. This is the one that actually holds for every present and future contract file, regardless of its own import graph; the subpath-vs-barrel distinction from 07-08 no longer matters once this is in place.

**How to apply:** don't trust `pnpm typecheck`/`pnpm test` green as proof a new *runtime value* import from `vendor/shared/contracts/*` actually loads in the app — `next.config.mjs`'s webpack resolution disagreed with `tsconfig.json`'s the whole time. Load the actual page (`next dev` + a real request, or `next build`) at least once after adding the first value-import of any given contract file.

**Evidence:** this session (2026-07-14); CI job `browser flows / Build + start web` on PR #28 failed identically to a local `pnpm dev` 500 — both `Module not found: Can't resolve './findings.js'`, trace `lib/hooks/multi-agent.ts` → `vendor/shared/contracts/observability.ts:2`; `client/tsconfig.json:5` (`moduleResolution:"Bundler"`); fixed via `client/next.config.mjs`'s `webpack()` hook; `pnpm build` and a live `curl localhost:3000/multi-agent` (200, compiled clean) both verified after the fix.
