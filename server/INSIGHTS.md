...
## 2026-07-13 — `ci_runs` (CI Runs / Export-to-CI) has NO repo column and no duration column; `source` is repurposed to carry the runner version

The `ci_runs` table + `CiRun` DTO (`server/src/db/schema/ci.ts`, `contracts/eval-ci.ts`) deliberately store NO `repo` column and NO duration column, and the free-text `source` column is repurposed by ingest to carry `CiResultArtifact.version` (so the agent CI tab's "workflow version" reads `CiRun.source` — the plan's "no schema change beyond `github_run_id`" decision). Consequences for any CI-runs UI/query: (1) the workspace CI Runs page derives `owner/name` by regex-parsing `github_url` (`github.com/owner/name/actions/runs/N`) — `repoFromUrl()` in `client/src/app/ci-runs/_components/CiRunsView/helpers.ts` — since there is no repo field; a repo filter/column needs a REAL additive `repo` column (+ re-mirror), not another URL parse. (2) `CiRun.duration_s` is ALWAYS null (no backing column) — UI shows "—". (3) `agent` is derived at read time via the installation→agent join, not stored. (4) Ingest dedupe/idempotency keys on the additive `github_run_id` unique index (migration 0022), not on `(installation, pr, ran_at)`. Also: `ci_installations` upsert is application-level select-then-write (no unique index on `(agent_id, repo)` — only `ci_runs.github_run_id` got one).

**Evidence:** this session (2026-07-13); `server/src/db/schema/ci.ts` (no repo/duration cols; `github_run_id` uniqueIndex), `server/src/modules/ci/service.ts` (`ingest` sets `source=version`, derives `agent` via join, app-level installation upsert), `client/src/app/ci-runs/_components/CiRunsView/helpers.ts` (`repoFromUrl`); architecture-reviewer PASS confirmed the URL-derivation is a deliberate limitation, not a bug.
