...
const inputSchema = {
  repo: z.string().min(1).describe('Repo full name, "owner/name" (see list_agents/get_conventions for examples).'),
  pr: z.number().int().positive().describe('Pull request number (not the internal id).'),
  agent: z.string().min(1).describe('Review agent name — see list_agents for available names.'),
};
...
async function fetchReviewOutcome(prId: string, runId: string, pr: number): Promise<ToolResult> {
  ...
  if (!res.ok) {
    return errorResult(`${res.error} Call get_findings with pr=${pr} to retry the read.`);
  }
  const review = res.data.find((r) => r.run_id === runId);
  if (!review) {
    return errorResult(
      `Review run ${runId} finished but no matching review row was found — call get_findings ` +
...
export const runReviewTool: ToolDef<typeof inputSchema> = {
  ...
  handler: async ({ repo, pr, agent }) => {
    ...
    if (!run) {
      return errorResult(
        'dev-digest API accepted the review trigger but returned no run — retry run_review, ' +
          `or call get_findings with pr=${pr} once a run appears.`,
      );
    }
    ...
    switch (wait.outcome) {
      ...
      case 'cancelled': {
        ...
        return errorResult(
          `Review run ${run.run_id} ${wait.outcome}${detail} — call get_findings with pr=${pr} ` +
            'to inspect any partial state, or retry run_review.',
        );
      }
      ...
      case 'error':
        ...
        return errorResult(
          `${wait.error} The review run ${run.run_id} may still be in progress — call ` +
            `get_findings later with pr=${pr} to check.`,
        );
    }
  },
};
